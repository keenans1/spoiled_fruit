describe('template spec', () => {
  ...
  it('Should see a list of titles that include my search when search button is clicked', () => {
    cy.get('form').within(() => {
      cy.get('input')
        .type('black')
        .get('button[class="search-button"]')
        .click()
    })
    cy.get('a[href="/movies/436270"]').should('exist')
  })

  it('Should render an error message when no movies match the search', () => {
    cy.get('form').within(() => {
      cy.get('input')
        .type('sdfghk')
        .get('button[class="search-button"]')
    ...
    cy.contains('Sorry, no results match your search')
  })
